...
int	mlx_put_image_to_window(t_xvar *xvar,t_window_list *window,t_img *img,
				int x,int y)
{
  GC	gc;
  ...
  if (img->type==MLX_TYPE_SHM)
    XShmPutImage(xvar->display,img->pix, window->gc, img->image,0,0,0,0,
		 img->width,img->height,False);
  if (img->type==MLX_TYPE_XIMAGE)
    XPutImage(xvar->display,img->pix, window->gc, img->image,0,0,0,0,
	      img->width,img->height);
  XCopyArea(xvar->display,img->pix,window->window, gc,
